...
The main result is contained in the file `02_Results/GalacticDWDBeaming.ecsv`. The `02_Results` folder contains the result from two simulations corresponding to two different eccentricity distributions, as explained in the article.

You will need to download the LSST baseline observation database file `baseline_v5.0.0_10yrs.db` and include in the the `01_Data` folder.

# SeBa DWD simulation
We used a modified version of SeBa. We did not change any of the physics and only modified the output writing of the package to limit file size during a large simulation. We can provide the steps to exactly modify the file upon request, but do not provide the modified package ourselves. SeBa files will stored within the folder `03_SeBa/package_SeBa`
